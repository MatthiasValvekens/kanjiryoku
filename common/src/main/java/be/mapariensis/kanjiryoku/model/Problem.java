...
public abstract class Problem implements Iterable<Word> {
     ...
     * 
     * @param sol
     *            The character list is a list of characters suggested by the
     *            guessing engine based on the user's input.
     * @return A number between 0 and the return value of
     *         {@code userInputCount()}.
     */
    ...
    public final boolean checkSolution(List<Character> sol, int pos) {
        ...
        if (relevantChar == 'ょ' || relevantChar == 'ゅ' || relevantChar == 'ゃ'
                || relevantChar == 'っ') {
            char alternativeChar = (char) (relevantChar + 1);
            return sol.contains(alternativeChar) || sol.contains(relevantChar); // most
                                                                                // OCR
                                                                                // engines
                                                                                // don't
                                                                                // distinguish
    ...
    public abstract String getFullSolution(); // TODO : model solutions as
                                                // something else, not strings
