...
public class MessageFragmentBuffer {
	private static final Logger log = LoggerFactory
			.getLogger(MessageFragmentBuffer.class);
	...
	public synchronized NetworkMessage postMessage(byte[] message, int offset,
			int length) {
		...
		if (message[offset + length - 1] == NetworkMessage.EOM) {
			...
			((ByteBuffer) buf.flip()).get(fullMessage); // read from pos zero to
														// current position
			String messageString = new String(fullMessage, Constants.ENCODING);
			readingPartialMessage = false;
			buf.clear();
			// assume only one message is in the buffer at one time (this is up
			// to the caller to enforce)
