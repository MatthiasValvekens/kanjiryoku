...
public class CompositeFilter<T> implements Filter<T> {
    public static enum CombinationMode {
        PERMISSIVE_MODE { // at least one check must pass (OR reduce)
            @Override
            public <T> boolean reduce(
                    Collection<? extends Filter<? super T>> filters, T thing) {
        ...
        public abstract <T> boolean reduce(
                Collection<? extends Filter<? super T>> filters, T thing);
    }
    ...
    public CompositeFilter(CombinationMode mode,
            Collection<? extends Filter<? super T>> filters) {
