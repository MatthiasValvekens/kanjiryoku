...
public class ServerConfigImpl implements ServerConfig {
	...
	public ServerConfigImpl(IPropertiesImpl config) throws BadConfigurationException {
		this.backend = config;
		loadExtra();
	}
	...
	private void loadExtra() throws BadConfigurationException {
		...
		IProperties crSettings = getRequired(ConfigFields.CR_SETTINGS_HEADER,IProperties.class);
		...
		IProperties psmSettings = getRequired(PSET_SETTINGS_HEADER,IProperties.class);
		int problemsPerCategory = psmSettings.getSafely(PROBLEMS_PER_CATEGORY, Integer.class,PROBLEMS_PER_CATEGORY_DEFAULT);
		boolean resetDifficulty = psmSettings.getSafely(RESET_AFTER_CATEGORY_SWITCH,Boolean.class,true);
		int minDiff = psmSettings.getSafely(MIN_DIFFICULTY, Integer.class, MIN_DIFFICULTY_DEFAULT);
		int maxDiff = psmSettings.getSafely(MAX_DIFFICULTY, Integer.class, MAX_DIFFICULTY_DEFAULT);
		String fileNameFormat = psmSettings.getRequired(FILE_NAME_FORMAT, String.class);
		String digitFormat = psmSettings.getSafely(FILE_NAME_DIFFICULTY_FORMAT, String.class,FILE_NAME_DIFFICULTY_FORMAT_DEFAULT);
		String enc = psmSettings.getSafely(FILE_ENCODING, String.class, FILE_ENCODING_DEFAULT);
		psm = new ProblemSetManagerImpl(fileNameFormat,digitFormat,problemsPerCategory, resetDifficulty, minDiff, maxDiff,enc);
		
		// load problem sets
		
		IProperties psconfig = psmSettings.getRequired(CATEGORY_LIST,IProperties.class);
		
		// iterate over the names of all problem sets
		for(String name : psconfig.keySet()) {
			psm.loadNewConfig(name,psconfig.getRequired(name,IProperties.class));
		}
	}
