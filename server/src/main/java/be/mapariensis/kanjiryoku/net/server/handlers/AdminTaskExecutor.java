...
public class AdminTaskExecutor extends ClientResponseHandler {
	...
	public void handle(User user, NetworkMessage msg) throws ServerException {
		if(msg.argCount()<3) throw new ArgumentCountException(Type.TOO_FEW, ServerCommand.ADMIN);
		int id;
		try {
		...
		if(!initiator.equals(user) || this.id != id) {
			throw new UserManagementException("Admin command issuer does not match responder or issue ID is incorrect. Aborting.");
		} else {
			task.run();
		}
	}
