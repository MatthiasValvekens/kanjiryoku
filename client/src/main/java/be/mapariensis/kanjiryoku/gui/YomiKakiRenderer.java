...
public abstract class YomiKakiRenderer implements ProblemRenderer {
	@Override
	public final void drawProblem(Graphics2D g, List<Character> correctInputs,
			Problem p, Character lastWrongInput) {
		...
		for (Word w : problem) {
			...
			if (problem.isBlank(w)) {
				...
				newpos = TextRendering.renderWord(g,
						getBlankMain(sequenced, w), getBlankRuby(sequenced, w),
						lastWrongInput == null ? null : getMistakeMarker());
			} else {
