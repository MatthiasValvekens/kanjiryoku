...
public class InputPanel extends JPanel implements InputHandler {
	...
	public void endProblem() {
		if (currentComponent != null) {
			...
			log.warn("No input component available, nothing to do.");
	}
	...
	public void receiveMessage(String user, NetworkMessage msg)
		...
		if (currentComponent != null
			...
			log.warn("Attempted to receive message, but no input handler was available.");
	...
	public void broadcastClearInput() {
		if (currentComponent != null
			...
			log.warn("Attempted to broadcast clear, but no input handler was available.");
	}
	...
	public void submit() {
		if (currentComponent != null
			...
			log.warn("Attempted submit, but no input handler was available.");
	}
	...
	public void prepareProblemPosition(Problem p, int position) {
		if (currentComponent != null
				&& currentComponent.getInputHandler() != null)
			currentComponent.getInputHandler().prepareProblemPosition(p,
					position);
		else
			log.warn("Attempted prepare, but no input handler was available.");
	}
