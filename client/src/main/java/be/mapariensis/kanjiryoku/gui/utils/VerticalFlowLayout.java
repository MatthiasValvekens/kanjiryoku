...
//grabbed from http://stackoverflow.com/questions/8196530/java-vertical-flowlayout-with-horizontal-scrolling
//first response
...
public class VerticalFlowLayout implements LayoutManager, java.io.Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 1757333792291076429L;
				...
				if (m.isVisible()) {
					...
					if ((y == 0) || ((y + d.height) <= maxHeight)) {
						...
						moveComponents(target, x, insets.top + vgap,
								columnWidth, maxHeight - y, start, i, ttb);
						y = d.height;
						x += hgap + columnWidth;
						columnWidth = d.width;
						start = i;
	...
	/*
	 * public static void main(String[] args) { JPanel main = new JPanel( new
	 * BorderLayout() );
	 * 
	 ...
	 * 
	 * for (int i = 0; i < 7; i++) { buttons.add( new JRadioButton("button " +
	 * i) ); }
	 * 
	 * JButton button = new JButton("Add Radio Button"); main.add(button,
	 * BorderLayout.SOUTH); button.addActionListener( new ActionListener() {
	 * private int i = 8;
	 * 
	 * public void actionPerformed(ActionEvent e) { buttons.add( new
	 * JRadioButton("button R Us" + i++) ); buttons.revalidate(); // pack(); }
	 * });
	 * 
	 * JFrame frame = new JFrame();
	 * frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); frame.add(main);
	 * frame.setSize(300, 300); frame.setLocationRelativeTo(null);
