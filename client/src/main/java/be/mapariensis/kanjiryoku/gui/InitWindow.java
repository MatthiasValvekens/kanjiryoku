	...
	private InitWindow() {
		...
		Action connectAction = new AbstractAction("Connect") {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					if(user.isEmpty()) throw new RuntimeException("User name empty");
					MainWindow wind = new MainWindow(InetAddress.getByName(host), Integer.valueOf(port.getText()), user);
					frame.setVisible(false);
					frame.dispose();
					wind.setVisible(true);
				} catch(RuntimeException ex) {
					JOptionPane.showMessageDialog(frame,"Illegal values in input.","Invalid input",JOptionPane.ERROR_MESSAGE);
					log.warn(ex.getMessage(),ex);
				} catch (UnknownHostException ex) {
					JOptionPane.showMessageDialog(frame,String.format("Unknown host \"%s\".",host),"Connection error",JOptionPane.ERROR_MESSAGE);
					log.warn(ex.getMessage(),ex);
				} catch (IOException ex) {
					JOptionPane.showMessageDialog(frame,"I/O error during connect.","Connection error",JOptionPane.ERROR_MESSAGE);
					log.warn(ex.getMessage(),ex);
				}
			}
		};
