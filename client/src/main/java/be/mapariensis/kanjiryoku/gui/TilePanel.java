...
public class TilePanel extends InputComponent implements
		MultipleChoiceInputInterface {
	private static final double TILE_SCALE = 0.9;
	private static final Color selectionColor = TextRendering.rgb(41, 82, 229,
			(float) 0.4);
	...
	private class Tile extends JComponent {
		...
		public void paintComponent(Graphics g) {
			...
			// TODO optimize for identical of tiles with the same content
			// length?
			if (f == null || !getSize().equals(size)) {
				fsize = contentClip.height / 2;
				FontMetrics fm;
				do {
					fm = g2d.getFontMetrics(f = new Font(
							TextRendering.FONT_NAME, Font.PLAIN, fsize--));
				} while ((swidth = fm.stringWidth(content)) > contentClip.width);
			}
			g2d.setFont(f);
			g2d.drawString(content, contentClip.x + contentClip.width / 2
					- swidth / 2, contentClip.y + contentClip.height / 2
					+ fsize / 3);
			size = getSize();
		}
	...
	public static Rectangle scale(Rectangle r, double factor) {
		...
		return new Rectangle(r.x + (r.width - newwidth) / 2, r.y
				+ (r.height - newheight) / 2, newwidth, newheight);
	}
