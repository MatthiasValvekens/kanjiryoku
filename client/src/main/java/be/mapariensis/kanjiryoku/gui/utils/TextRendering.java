...
public class TextRendering {
	...
	public static int renderWord(Graphics2D g2d, String main, String furigana,
		...
		int width = Math.max(rubyWidth, mainWidth) + 5; // include a few pixels
														// for padding
		log.debug("Widths: Ruby: {}, Normal: {}", rubyWidth, mainWidth);
		if (mainWidth > rect.width)
			log.warn("Clip too small to contain text ({} pixels short).",
	...
	public static Color rgb(int r, int g, int b, float alpha) {
		return new Color((float) (r / 256.0), (float) (g / 256.0),
				(float) (b / 256.0), alpha);
	}
}
